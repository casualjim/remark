...
impl Backend {
  ...
  async fn publish_diagnostics(&self, uri: &Url) {
    ...
    let review = match load_file_review(&repo, &self.notes_ref, &rel_path, self.base_ref.as_deref())
    {
      Ok(review) => review,
      Err(err) => {
        self.log_error(err).await;
  ...
  async fn hover_for(&self, uri: &Url, position: Position) -> Option<Hover> {
    ...
    let review =
      load_file_review(&repo, &self.notes_ref, &rel_path, self.base_ref.as_deref()).ok()??;
  ...
  async fn execute_remark_command(&self, command: &str, args: ResolveArgs) {
    let side = match args.side.as_deref() {
      ...
      Some(other) => {
        self
          .log_error(anyhow::anyhow!("invalid side '{other}'"))
          .await;
        return;
      }
      None => None,
  ...
  async fn sync_draft_if_needed(&self, uri: &Url) -> bool {
    ...
    if let Err(err) =
      crate::add_cmd::sync_draft_notes_on_save(&repo, &self.notes_ref, self.base_ref.as_deref())
    {
      self.log_error(err).await;
    }
...
impl LanguageServer for Backend {
  ...
  async fn inlay_hint(&self, params: InlayHintParams) -> LspResult<Option<Vec<InlayHint>>> {
    ...
    Ok(Some(
      self
        .inlay_hints_for(&params.text_document.uri, params.range)
        .await,
    ))
  }
  ...
  async fn code_action(
    ...
    Ok(Some(
      self
        .code_actions_for(&params.text_document.uri, params.range.start.line)
        .await,
    ))
  }
...
mod tests {
  ...
  fn init_repo_with_commit(path: &str, contents: &str) -> (tempfile::TempDir, gix::Repository) {
    ...
    repo
      .reference("HEAD", commit_id, PreviousValue::Any, "test commit")
      .expect("update HEAD");
  ...
  fn write_file_note(
    ...
    let key = crate::git::note_file_key_oid(repo, head, view, base_ref, path).expect("note key");
  ...
  async fn lsp_end_to_end_diagnostics_and_hover() {
    ...
    .await;
    let _init = wait_for_message(&mut client_read, |msg| msg.get("id") == Some(&1.into())).await;
    ...
    .await;
    let _exec = wait_for_message(&mut client_read, |msg| msg.get("id") == Some(&5.into())).await;
