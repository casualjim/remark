...
pub(crate) fn write_draft_from_review(
  ...
  for (path, file) in &review.files {
    for (key, comment) in &file.comments {
      ...
      if let Some(hash) = comment.snippet_hash.as_ref() {
        preferred_hashes.insert(meta_key(path, side_label(key.side), key.line), hash.clone());
      }
    }
  }
  rebuild_draft_meta(
    ...
    if let Some(rest) = trimmed.strip_prefix("```") {
      ...
      if lang.starts_with("text") {
        pending_target = match mode {
          ...
          DraftSection::LineComments => current_file
            .clone()
            .and_then(|path| pending_line.map(|(line, side)| DraftTarget::Line(path, line, side))),
